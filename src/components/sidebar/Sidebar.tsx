...
import ChatLinks from '../log/ChatLinks';
...
const Sidebar = ({
  ...
  >(null);
  const [chatLinksData, setChatLinksData] = useState<{
    userLog: LogEntry;
    userAvatarId: number;
    userName: string;
  } | null>(null);
  ...
  const handleChatLinksClick = (
    userLog: LogEntry,
    userAvatarId: number,
    userName: string
  ) => {
    setChatLinksData({ userLog, userAvatarId, userName });
    setActivePanel('chatlinks');
  };

  const handleChatLinksBack = () => {
    setActivePanel('log');
    setChatLinksData(null);
  };
  ...
  return (
    <>
      ...
      <div className={`sidebar-panel ${activePanel ? 'is-open' : ''}`}>
        ...
        {activePanel === 'chatlinks' && chatLinksData && (
          <ChatLinks
            userLog={chatLinksData.userLog}
            userAvatarId={chatLinksData.userAvatarId}
            userName={chatLinksData.userName}
            onBack={handleChatLinksBack}
          />
        )}
      </div>
    </>
